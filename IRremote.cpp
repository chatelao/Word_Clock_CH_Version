...
#ifdef DEBUG
  Serial.printf(F("Attempting NEC decode\n"));
#endif
...
int16_t IRrecv::getRClevel(decode_results *results, int16_t *offset, int16_t *used, int16_t t1) 
  ...
  if (val == MARK) 
    {
    Serial.printf(F("MARK\n"));
    } 
  else 
    {
    Serial.printf(F("SPACE\n"));
    }
#endif
  return val;   
  }
...
int32_t IRrecv::decodeLG(decode_results *results) 
  {
  int32_t data = 0;
  int16_t offset = 1; // Skip first space
...
//------------------------------------------------------------------------------------------------------------
int32_t IRrecv::decodeSAMSUNG(decode_results *results) 
  {
  int32_t data = 0;
  int16_t offset = 1; // Skip first space
  // Initial mark
  ...
  offset++;
  for (int16_t i = 0; i < SAMSUNG_BITS; i++) 
    {
